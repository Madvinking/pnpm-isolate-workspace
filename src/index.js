...
async function createPnpmLock() {
  ...
  let recuireDeps = await lockfile.readWantedLockfile(pnpmLockPath, 'utf8');

  recuireDeps.importers['.'] = JSON.parse(JSON.stringify(recuireDeps.importers[workspaceData.name]));
  delete recuireDeps.importers[workspaceData.name];

  Object.keys(recuireDeps.importers).forEach(key => {
    if (!relatedWorkspaces[key]) delete recuireDeps.importers[key];
  });
  ...
  const listOfDeps = Object.values(recuireDeps.importers).reduce((acc, { dependencies }) =>
    [...new Set([...acc, ...getDepsList(dependencies)])]
    , []);
